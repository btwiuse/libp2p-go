...
Start [mdns discovery](https://godoc.org/github.com/webtransport/libp2p-go/p2p/discovery#NewMdnsService) service in host.

```go
ser, err := discovery.NewMdnsService(peerhost, rendezvous)
```
register [Notifee interface](https://godoc.org/github.com/webtransport/libp2p-go/p2p/discovery#Notifee) with service so that we get notified about peer discovery
